...
/***                                                            ***/
/***                         = TP =                             ***/
/***      Implementation d'une fonction d'estimation estim7     ***/
/***  capable a vaincre les sept autres fonctions implementees  ***/
/***                   - lignes 1151-1333                       ***/
/***                                                            ***/
/***                                                            ***/
/***                                                            ***/
...
/* Notre fonction d'estimation implementee */
int estim7( struct config *conf )
{
	int i, j, ScrQte,Score;
	int phase=1;
	int bonus_centralisation =0, bonus_flan =0, bonus_positionB = 0, bonus_positionN = 0, bonus_position= 0;
	int nbr_piecesB = 0, nbr_piecesN = 0, nbr_pieces_total = 0 ;
	int pionB = 0, pionN = 0, cB = 0, cN = 0, fB = 0, fN = 0, tB = 0, tN = 0, nB = 0, nN = 0 , rB=0,rN=0;
	int pionB_mob = 0, pionN_mob = 0, cB_mob = 0, cN_mob = 0, fB_mob=0,fN_mob=0, tB_mob = 0, tN_mob = 0, nB_mob = 0, nN_mob = 0 ,rB_mob = 0, rN_mob = 0;
	int pionIsole = 0, pionDouble = 0;

	int nbr_pion_doubleB=0;  // nombre de pions doublés  ( 2 pions qui se suivent dans la meme colonne)
	int nbr_pion_doubleN=0;
	int nbr_pion_double =0;  // la difference entre Blans et Noirs

	int nbr_pion_isoleB=0;   // nombre de pions isolés ( si un pion n'a pas de pion voisin dans les colonnes voisines )
	int nbr_pion_isoleN=0;
	int nbr_pion_isole=0;    // la difference entre Blans et Noirs


    int degLiberte = 0;
	struct config T[100];

	int material =0; // presente le nombre de pieces pondererees avec ses poids
	int mobility =0; // degre de liberte de deplacement des pieces

	// Definir la phase du jeu : debut , milieu ou approche du fin du jeu
	// on exploite ce parcour aussi pour calculer le bonus de centralisation pour les pions
	// on exploite ce parcour aussi pour calculer le bonus de deploiement sur les flans : Methode detaillee dans le rapport
	for (i=0; i<8; i++)
	   for (j=0; j<8; j++) {
        bonus_centralisation=0;
        bonus_flan = 0;

		if (i>1 && i<6 && j>=0 && j<=7 ) bonus_centralisation = 1;
		if (i>2 && i<5 && j>=2 && j<=5 ) bonus_centralisation = 2;
		if (j>4 && j<3 )bonus_flan=2;
		switch (conf->mat[i][j]) {
		   case 'p' : nbr_piecesB++; bonus_positionB+=bonus_centralisation; break;
		   case 'c' : nbr_piecesB++; bonus_positionB+=3*bonus_centralisation+bonus_flan;break;
		   case 'f' : nbr_piecesB++; bonus_positionB+=3*bonus_centralisation+bonus_flan;break;
		   case 't' : nbr_piecesB++; bonus_positionB+=5*bonus_centralisation+bonus_flan;break;
		   case 'n' : nbr_piecesB++; bonus_positionB+=9*bonus_centralisation;break;

		   case -'p' : nbr_piecesN++; bonus_positionN+=bonus_centralisation; break;
		   case -'c' : nbr_piecesN++; bonus_positionN+=3*bonus_centralisation+bonus_flan;break;
		   case -'f' : nbr_piecesN++; bonus_positionN+=3*bonus_centralisation+bonus_flan;break;
		   case -'t' : nbr_piecesN++; bonus_positionB+=5*bonus_centralisation+bonus_flan;break;
		   case -'n' : nbr_piecesN++; bonus_positionB+=9*bonus_centralisation;break;
		}
	   }
	// calcul de bonus de positionnement
	bonus_position= bonus_positionB - bonus_positionN;
	// max = 57

	nbr_pieces_total= nbr_piecesB+nbr_piecesN;

	//contraintes du choix de la phase du jeu
	// on a besoin de connaitre la phase pour choisir la strategie a jouer
    if(num_coup<9 || nbr_pieces_total>29){phase=1;}  // phase debut du jeu
    else{
        if(nbr_piecesB<4 || nbr_piecesN<4){phase=3;}  // phase approche du fin de jeu
        else {phase=2;}                         // phase milieu du jeu
    }
	...
	for (i=0; i<8; i++)
	   for (j=0; j<8; j++) {
		...
		switch (conf->mat[i][j]) {
		   // joueur avec couleur de pieces noirs
		   case 'p' : pionB++;
                      degLiberte=0;
                      deplacementsB(conf, i, j, T,&degLiberte);
                      pionB_mob+=degLiberte;
                      if((conf->mat[i-1][j]== 'p')||(conf->mat[i+1][j]== 'p')) nbr_pion_doubleB++;
                      if((conf->mat[i][j-1]== 'p')||(conf->mat[i][j+1]== 'p')) nbr_pion_isoleB++;
                      break;
		   ...
		   case -'p' : pionN++;
                       degLiberte=0;
                       deplacementsN(conf, i, j, T,&degLiberte);
                       pionN_mob+=degLiberte;
                       if((conf->mat[i-1][j]== -'p')||(conf->mat[i+1][j]== -'p')) nbr_pion_doubleN++;
                       if((conf->mat[i][j-1]== -'p')||(conf->mat[i][j+1]== -'p')) nbr_pion_isoleN++;
                       break;
	...
	//materials  : on associe a chaque piéce un coefficient designant son poid
	material = (pionB*1 + cB*3 +fB*3 + tB*5 + nB*9) - (pionN*1 + cN*3 + fN*3 + tN*5 + nN*9);
    //max=39

    //mobility  : definit le nombre de deplacement possible pour chaque piece
    mobility = ( (pionB_mob + cB_mob +fB_mob + tB_mob + nB_mob +rB_mob ) - (pionN_mob + cN_mob + fN_mob + tN_mob + nN_mob + rN_mob ) );

    //pions doublees et pions isolees sont des criteres de faiblesse
    nbr_pion_double = nbr_pion_doubleN - nbr_pion_doubleB;
    nbr_pion_isole = nbr_pion_isoleN - nbr_pion_isoleB;

    //  calcul du score total
    if(phase==1 || phase==3){
        // si on est a la phase debut ou fin on tente de centraliser pour dominer le jeu
        // ici le positionnement de pions compte beaucoup soit en attaquant ou en liberant les autres pieces
        ScrQte = material;
        Score = (4*ScrQte + bonus_position) * 100.0/(4*39+57);
    }else{
        // si on est a la phase milieu du jeu , on favorise la mobilite des pieces
        ScrQte = (material*10 + mobility + nbr_pion_double*5 + nbr_pion_isole*5)/10;
        bonus_position=bonus_position/57;
        Score = (4*ScrQte +bonus_position)/5;
    }
